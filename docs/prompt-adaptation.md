...
| Tines skills | Files under `skills/<name>/…`. | Not seeded. The agent may inspect the Tines context API/CLI if a required skill is absent, subject to the separate skill-forwarding design. | Forward selected skill files with their names, descriptions, and byte limits, or make them available through an explicit Cloud-native mechanism. Never claim a local path exists when it does not. |
